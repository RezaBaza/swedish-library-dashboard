...
class HybridQA:
    ...
    def load_data(self, csv_path):
        ...
        print(f"Data loaded successfully. DataFrame shape: {self.df.shape}")
        print(f"Columns: {self.df.columns.tolist()}")
        print(f"Sample regions: {self.df['Region_Name'].unique()[:5]}")
        print(f"Year range: {self.df['Year'].min()} to {self.df['Year'].max()}")
